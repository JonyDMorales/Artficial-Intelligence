  ...
  (let  ( (newX (+ (first estado) (first (second op)))) (newY (+ (second estado) (second (second op))))
  	...
  	(cond ((not (and (>= newX 0) (< newX *limX*) (>= newY 0) (< newY *limY*))) nil)
  	  (T (setq p (get-cell-walls (first estado) (second estado)) d (get-cell-walls newX newY))
  	  	...
  	  	;(format t "~a ~a~%" p d)
  	  	(cond
  	  	  ((eql  (first op) :2Arriba)
  	  	  	(setq d (get-cell-walls (+ newX 1) newY))
  		  	(or (= d 16) (= d 17)))
  	  	  ((eql  (first op) :2Derecha)
  	  	  	(setq d (get-cell-walls newX (- newY 1)))
  		  	(or (= d 16) (= d 17)))
  	  	  ((eql  (first op) :2Izquierda)
  	  	  	(setq d (get-cell-walls newX (+ newY 1)))
  		  	(or (= d 16) (= d 17)))
  	  	  ((eql  (first op) :2Abajo)
  	  	  	(setq d (get-cell-walls (- newX 1) newY))
  		  	(or (= d 16) (= d 17)))
  		  ((eql  (first op) :Arriba)
  		  	(and (/= d 16) (/= d 17) (/= (logand p 1) 1)))
  		  ((eql  (first op) :Derecha)
	      	(and (/= d 16) (/= d 17) (/= (logand p 2) 2)))
  		  ((eql  (first op) :Abajo)
	      	(and (/= d 16) (/= d 17) (/= (logand p 4) 4)))
  		  ((eql  (first op) :Izquierda)
	      	(and (/= d 16) (/= d 17) (/= (logand p 8) 8)))
